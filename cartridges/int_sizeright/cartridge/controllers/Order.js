...
server.post(
    ...
    function (req, res, next) {
        ...
        var trackingEmailId = order.customerEmail;
        var trackingOrderId = order.orderNo;
        var trackingOrderTotal = order.totalGrossPrice.value;
        var trackingProductIds = order.productLineItems;
        var trackingProductIdsArray = [];
        collections.forEach(trackingProductIds, function (product) {
            trackingProductIdsArray.push(product.productID);
        });
        ...
        var trackingObject = {
            ...
            product_id: trackingProductIdsArray[0],
        ...
        if (false && lastOrderID === req.querystring.ID) {
        ...
        var pixeltracking = require('*/cartridge/scripts/hooks/services/pixelTrackingService').orderPixelTracking.call(trackingObject);
        ...
        if (false && !req.currentCustomer.profile) {
